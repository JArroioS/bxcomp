...
bool palin(char *texto, int posicaoInicial, int posicaoFinal) //Assinatura da função que recebe como parâmetros uma string de 100 caracteres, as posições iniciais e finais do que queremos verificar se é palindromo
{

    //O código abaixo irá iterar e comparar os caracteres nos sentidos "inicial até final" e "final até inicial" (primeiro com o último, segundo com penúltimo, terceiro com antepenúltimo, ..., meio com meio)
...
int main()
{
    ...
    //verificacao = false; // por algum motivo deixar essa linha dá um erro sinistro! Mas como a função palin sempre retorna um valor não há problema em não rodá-la

    char * string; // string que iremos utilizar como instância da nossa função
    scanf("%[^\n]%*c", string);

    int tamanhoString = strlen(string); // variável utilizada para receber o tamanho da nossa string
    ...
    for(int i = 0; i < (tamanhoString-2); i++) // iteração de todos os caracteres do primeiro caractére até o anterior do antepenúltimo, visto que um palindromo possui ao menos 3 letras (ex: Em "PARARABO" a iteração será: P > A > R > A > R > A)
    {
        ...
        for(int j = tamanhoString-1; j > (i+1); j--) // iteração dos caractéres do final até i+1 (é preciso ser i+1 visto que um palíndromo possui ao menos 3 letras, logo não podemos verificar o valor seguido do i e muito menos o próprio i) (ex: Em "PARARABO", se i = 2 a iteração será: O > B > A > R)
        {
            if(string[i] == string[j]) // verifica se o caractere da primeira iteracao é igual ao da segunda, caso sejam iguais esses caracteres são respectivamente o primeiro e o último do nosso possível palindromo (ex: Em "PABREARABO" os primeiros i e j que iriam dariam a comparação como verdadeira seriam 1 e 7 (ABREARA), embora o palindromo esteja delimitado apenas entre 5 e 7(ABA) )
            {
                ...
                //Se tiver um palindromo na string esse bloco imprime que tem um palindromo e para a execução do código
                if(verificacao == true)
                {
    ...
    //Se o código chegou até aqui então a string não possui um palindromo
